...
QString DecryptVerifyResult::overview() const
{
    ...
    if (isNotepad()) {
        if (decrypting && d->m_decryptionResult.error()) {
            return i18nc("@info", "Failed to decrypt the notepad's content: %1", Formatting::errorAsString(d->m_decryptionResult.error()));
        }
        if (verifying && d->m_verificationResult.error()) {
            return i18nc("@info", "Failed to verify the notepad's content: %1", Formatting::errorAsString(d->m_verificationResult.error()));
        }
