...
QString SignEncryptFilesResult::overview() const
{
    if (isNotepad()) {
        if (!m_sresult.isNull() && m_sresult.error()) {
            return i18nc("@info", "Failed to sign the notepad's content: %1", Formatting::errorAsString(m_sresult.error()));
        }
        if (!m_eresult.isNull() && m_eresult.error()) {
            return i18nc("@info", "Failed to encrypt the notepad's content: %1", Formatting::errorAsString(m_eresult.error()));
        }
